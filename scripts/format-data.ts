...
import { loadChangesFile, loadInstruments, loadNotationFile, loadQualitiesFile, loadScalesFile, ROOT } from './data-source';

const check = process.argv.includes('--check');
const unformatted: string[] = [];

const files = [loadQualitiesFile(), loadScalesFile(), loadChangesFile(), loadNotationFile(), ...loadInstruments().flatMap((s) => [s, ...s.chords])];
