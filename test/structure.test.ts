...
import { keyDir, loadChangesFile, loadInstruments, loadNotationFile, loadQualitiesFile, suffixSlug } from '../scripts/data-source';
...
describe('all data', () => {
  ...
  it('is formatted canonically (npm run format:data)', () => {
    const unformatted = [loadQualitiesFile(), loadChangesFile(), loadNotationFile(), ...instruments.flatMap((source) => [source, ...source.chords])]
